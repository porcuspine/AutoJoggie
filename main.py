...
class Autoclicker:
    ...
    def __update__(self, deltaMs):
        if self.is_paused() or len(self._clicksQueue) == 0: return
    ...
    def start(self, waitTimeMs:float, waitVariMs:float, numClicks:int = -1, doDoubleClick:bool = False):
    ...
    def pause(self, setPause:bool):
        ...
        if self._isPaused:
            if len(self._clicksQueue) > 0 and self._clicksQueue[0].is_click_up(): #if user paused between 'down' and 'up'
                mouseUp()
                self._clicksQueue.clear()
                self._decrement_clickcount()
        else:
    ...
    def _queue_click(self, timeMs:float|None = None):
        timeUntil = timeMs if timeMs is not None else (self._waitTimeMs + random.uniform(0, self._waitVariMs))
        holdTime = random.uniform(*self.MOUSE_CLICK_LENGTH_RANGE_MS)
        self._clicksQueue.append(ClickTimer(timeUntil, True))
...
class Window:
    ...
    def __init__(self):
        ...
        self._pauseKeybind = Keybinder(defaultKey=keyboard.Key.home)
        self._stopKeybind = Keybinder(defaultKey=keyboard.Key.end)
    ...
    def __update__(self):
        ...
        timeToClickStr = "Now!" if nextClick is None or nextClick.is_click_up() else f"{nextClick.get_time()/1000:.2f}s"
        if self._autoclicker.is_paused():
            if self._autoclicker.get_clicks() != -1:
                readoutText = f"{self._autoclicker.get_clicks()} clicks left.\n(Clicking is paused)"
            else:
                readoutText = f"(Clicking is paused)\n"
        elif self._autoclicker.get_clicks() < 0:
            readoutText = f"Clicking infinitely!\nNext click: {timeToClickStr}"
        else:
            readoutText = f"{self._autoclicker.get_clicks()} clicks left.\nNext click: {timeToClickStr}"
        self.readoutLabel.config(text=readoutText)
    ...
    def _try_start(self):
        try:
            numClicks = self.numClicksField.get()
        ...
        self._autoclicker.start(waitTimeMs, waitVariMs, numClicks, doDoubleClick)
